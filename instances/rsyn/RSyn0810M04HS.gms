*Sawaya epsilon reformulation G(x,z) = ((1-es)*z+es)*g(x / ((1-es)*z+es) ) - es*g(0)*(1-z)*Retrofit-Synthesis Problem with 8 retrofit processes and 10 synthesis processes
*Four Periods (t=4)
*Convex-Hull Version
...
PARAMETER UB(K,D,I,T) /1*25 .1*2 .1*10 .1*4= 0/;
...
MODEL RETRO_8_SYNTH_10_MULTI_CH /ALL/;
OPTION LIMROW = 0;
OPTION LIMCOL = 0;
$if not set RTYPE $set RTYPE rMINLP
$if not set TYPE $set TYPE MINLP
SOLVE RETRO_8_SYNTH_10_MULTI_CH USING %TYPE% MAXIMIZING OBJ;
